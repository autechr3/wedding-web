...
export function Nav() {
  ...
  return (
    <nav className="sticky top-0 z-20 flex h-12 items-center justify-between bg-cobalt-deep/95 px-4 backdrop-blur border-b border-gold-soft/25">
      <span className="font-crest tracking-[0.25em] text-gold-soft text-sm">N · M</span>
      <span className="flex items-center gap-3">
        <button type="button" aria-label="English" aria-pressed={locale === 'en'} onClick={() => setLocale('en')} className={`cursor-pointer ${focus}`}>
          <Flag country="us" className={flag(locale === 'en')} />
        </button>
        <button type="button" aria-label="فارسی" aria-pressed={locale === 'fa'} onClick={() => setLocale('fa')} className={`cursor-pointer ${focus}`}>
